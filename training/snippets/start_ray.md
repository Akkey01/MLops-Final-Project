...
```bash
# run on node-mltrain
docker build -t ray-rocm:2.42.1 -f MLOps-Final-Project/training/docker/Dockerfile.ray-rocm .
```

It will take 5-10 minutes to build the container image.

You can see this Dockerfile here: [Dockerfile.ray-rocm](https://github.com/teaching-on-testbeds/MLOps-Final-Project/training/blob/main/docker/Dockerfile.ray-rocm).
...
docker compose -f MLOps-Final-Project/training/docker/docker-compose-ray-rocm.yaml up -d
```

You can see this Docker Compose YAML here: [docker-compose-ray-rocm.yaml](https://github.com/teaching-on-testbeds/MLOps-Final-Project/training/blob/main/docker/docker-compose-ray-rocm.yaml).
...
# run on node-mltrain
export HOST_IP=$(curl --silent http://169.254.169.254/latest/meta-data/public-ipv4 )
docker compose -f MLOps-Final-Project/training/docker/docker-compose-ray-cuda.yaml up -d
```

You can see this Docker Compose YAML here: [docker-compose-ray-cuda.yaml](https://github.com/teaching-on-testbeds/MLOps-Final-Project/training/blob/main/docker/docker-compose-ray-cuda.yaml).
...
```bash
# run on node-mltrain
docker build -t jupyter-ray -f MLOps-Final-Project/training/docker/Dockerfile.jupyter-ray .
```
...
docker run  -d --rm  -p 8888:8888 \
    -v ~/MLOps-Final-Project/training/workspace_ray:/home/jovyan/work/ \
